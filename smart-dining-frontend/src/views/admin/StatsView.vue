<template>
  <div class="stats-view">
    ...
    <div class="page-content">
      ...
      <!-- 每日数据 -->
      <h3 class="section-title">{{ isZh ? '每日数据' : 'Daily Data' }}</h3>
      <div class="daily-list">
        <div v-for="stat in salesStats" :key="stat.date" class="daily-item">
          <div class="daily-date">{{ formatDate(stat.date) }}</div>
          <div class="daily-data">
            <span>{{ stat.total_orders }} {{ isZh ? '单' : 'orders' }}</span>
            <span class="daily-revenue">¥{{ stat.total_revenue.toFixed(2) }}</span>
          </div>
        </div>
      </div>
...
const totalRevenue = computed(() => salesStats.value.reduce((s, d) => s + d.total_revenue, 0))
...
function formatDate(d) { return d.slice(5) }
...
.daily-item { display: flex; justify-content: space-between; padding: var(--space-md); border-bottom: 1px solid var(--color-border); }
.daily-item:last-child { border-bottom: none; }
.daily-date { color: var(--color-text-secondary); }
